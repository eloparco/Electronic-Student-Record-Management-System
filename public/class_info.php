...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "SELECT DISTINCT TEACHER_SUBJECT.Class, USER.SSN FROM USER, TEACHER_SUBJECT, SUBJECT WHERE SUBJECT.ID = TEACHER_SUBJECT.SubjectID AND USER.SSN = TEACHER_SUBJECT.TeacherSSN AND USER.Email = ?";
    ...
    mysqli_stmt_bind_result($prep_query, $Class, $SSN);

    $rows = array();

    while (mysqli_stmt_fetch($prep_query)) {
        //echo $Class.$Name.$ID.$SSN;
        $fields = array("Class" => $Class, "SSN" => $SSN);
        $classes[] = $fields;
